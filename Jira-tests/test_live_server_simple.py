#!/usr/bin/env python3
"""
Simple Live MCP Server Test Script
This script uses basic HTTP requests to test your running MCP server
without requiring additional MCP client dependencies.
"""
...
try:
    ...
    try:
        import requests
        ASYNC_MODE = False
        print("ℹ️  Using synchronous requests (install aiohttp for async mode)")
    except ImportError:
        print("❌ No HTTP library available. Please install either:")
...
class SimpleMCPTester:
    """Simple MCP server tester using basic HTTP requests."""
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        if ASYNC_MODE and self.session:
            await self.session.close()
    
    async def make_request(self, method: str, url: str, **kwargs) -> Dict[str, Any]:
        """Make an HTTP request (async or sync)."""
        if ASYNC_MODE:
            return await self._make_async_request(method, url, **kwargs)
        else:
            return self._make_sync_request(method, url, **kwargs)
    ...
    async def _make_async_request(self, method: str, url: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status_code": 0, "text": str(e), "json": None, "error": str(e)}
    ...
    async def test_server_health(self) -> bool:
        ...
        try:
            ...
            response = await self.make_request("GET", f"{self.server_url}/healthz")
            ...
            response = await self.make_request("GET", self.mcp_url)
    ...
    async def call_mcp_tool(self, tool_name: str, arguments: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            print_error(f"MCP tool call failed for {tool_name}: {e}")
            raise
    ...
    async def test_jira_get_all_projects(self) -> bool:
        ...
        try:
            result = await self.call_mcp_tool("jira_get_all_projects", {
    ...
    async def test_jira_search(self) -> bool:
        ...
        try:
            result = await self.call_mcp_tool("jira_search", {
    ...
    async def test_jira_get_project_issues(self) -> bool:
        ...
        try:
            result = await self.call_mcp_tool("jira_get_project_issues", {
                "project_key": "AI",
                "limit": 3
            })
    ...
    async def run_all_tests(self) -> bool:
        """Run all tests."""
        print_section("Simple Live MCP Server Test")
        print_info(f"Testing server at: {self.server_url}")
        print_info(f"Using {'async' if ASYNC_MODE else 'sync'} HTTP requests")
...
async def main():
    ...
    if ASYNC_MODE:
        async with SimpleMCPTester(server_url) as tester:
...
if __name__ == "__main__":
    try:
        if ASYNC_MODE:
            ...
            # For sync mode, we still need to handle the async functions
            import asyncio
            exit_code = asyncio.run(main())
        sys.exit(exit_code)
